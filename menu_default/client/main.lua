...
ESXMenu.UI.Menu.CloseAll = function()
    ...
    -- Убедимся, что курсор скрыт при закрытии всех меню
    if menuVisible then
...
Citizen.CreateThread(function()
	...
	local closeMenu = function(namespace, name)
		...
		-- Проверяем, осталось ли открытым хотя бы одно меню
		local menuCount = 0
		for i=1, #ESXMenu.UI.Menu.Opened, 1 do
			if ESXMenu.UI.Menu.Opened[i] then
		...
		-- Если все меню закрыты, скрываем курсор
		if menuCount <= 1 and menuVisible then  -- <= 1 потому что текущее меню ещё не удалено
			SetNuiFocus(false, false)
			menuVisible = false
		end
	end
	...
	RegisterNUICallback('menu_cancel', function(data, cb)
		...
		if not data or not data._namespace or not data._name then
			print("Invalid menu data received in menu_cancel")
			SetNuiFocus(false, false)
			menuVisible = false
			cb('ERROR')
			return
		...
		-- Try to get the menu with nil check
		local menu = ESXMenu.UI.Menu.GetOpened(MenuType, data._namespace, data._name)
		
		-- Close the menu even if we can't get a reference to it
		ESXMenu.UI.Menu.Close(MenuType, data._namespace, data._name)
	
		-- If we have a valid menu reference, call its cancel callback
		if menu and menu.cancel then
			menu.cancel(data, menu)
		end
	
		-- Check remaining open menus
		local menuCount = 0
		for i=1, #ESXMenu.UI.Menu.Opened, 1 do
			if ESXMenu.UI.Menu.Opened[i] then
				menuCount = menuCount + 1
		...
		-- If no menus are left open, disable NUI focus
		if menuCount == 0 and menuVisible then
			SetNuiFocus(false, false)
			menuVisible = false
		end
